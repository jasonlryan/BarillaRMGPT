  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath)
      ...
      .on("data", (row) => {
        ...
        if (firstColumn.includes("jrn_trigger:")) {
          currentSection = "jrn_trigger";
          return;
        } else if (firstColumn.includes("ctm_occasion_in_mind:")) {
          currentSection = "ctm_occasion_in_mind";
          return;
        }

        // Skip empty rows or rows without valid data
        if (!firstColumn || !currentSection) {
          return;
        }
        ...
        results[currentSection][firstColumn] = dataPoint;
      })
      .on("end", () => {
        resolve(results);
      })
