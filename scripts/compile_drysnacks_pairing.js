...
function compileData(filePath) {
  return new Promise((resolve, reject) => {
    const results = {
      _instructions: {
        description: "Dry snacks pairing preferences for Italian market",
        dataCharacteristics: {
          marketCoverage: {
            core: ["Italy"],
          },
          dataHandling: {
            rules: [
              ...
              "Net relative values preserved for each pairing",
            ],
          },
        },
      },
    ...
    fs.createReadStream(filePath)
      ...
      .on("data", (row) => {
        // Get the pairing text from the first column
        const pairing = Object.values(row)[0];
        if (!pairing || pairing.includes("ctm_planned_pairing:")) return;
        ...
        results.ctm_planned_pairing[pairing] = {
          Italy: responseRate,
        };
      })
      .on("end", () => {
...
// Main execution
const inputPath = path.join(__dirname, "data", "drysnacks_pairing.csv");
const outputPath = path.join(__dirname, "vector", "drysnacks_pairing.json");
